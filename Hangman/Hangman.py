...
def input_ok(word, guessed_letters, guessed_words):
    try_user = input('Введите букву или слово целиком:').upper()
    while (1 < len(try_user) < len(word)) or len(try_user) > len(word) and try_user.isalpha() == False:
        print('Введено некорректное слово или символ')
        try_user = input('Введите букву или слово целиком:').upper()

    while try_user in guessed_letters or try_user in guessed_words:
        print('Вы уже вводили данную букву или слово')
        try_user = input('Попробуйте снова: ').upper()
        while (1 < len(try_user) < len(word)) or len(try_user) > len(word) and try_user.isalpha() == False:
            print('Введено некорректное слово или символ')
            try_user = input('Введите букву или слово целиком:').upper()
    return try_user
...
def play(word):
    ...
    print('Я хочу сыграть с вами в игру "Угадай слово"')
    ...
    while tries != 0:
        try_user = input_ok(word, guessed_letters, guessed_words)
        if len(try_user) == 1:
            guessed_letters.append(try_user)
        else:
            guessed_words.append(try_user)
        if try_user in word and len(try_user) == 1:
            for j in range (len(word)):
                if try_user == word[j]:
                    word_completion = word_completion[: 2 * j] + \
                        try_user + ' ' + word_completion[2 * (j + 1):]
            print('Вы угадали!')
            print(word_completion)
        elif try_user == word:
            print('Вы угадали слово ! Женя спасен !',
            ...
            if tries != 0:
                print(
                    'Промах ! Женя все ближе к эшафоту (. Осталось попыток:', tries, 'Загаданное слово', word_completion)
        print(display_hangman(tries))
        if tries == 0:
            print('Вы проиграли ! Женя Удавился ! Загаданное слово:', word)
            break
        if '_ ' not in word_completion and tries != 0:
            print('Поздравляем, вы выйграли. Женя спасен !', '(Загаданное слово:', word_completion, ')')
            break

play(get_word())
again = input('Сыграть еще раз ?:').lower()
while again == 'да':
    play(get_word())
    again = input('Сыграть еще раз ?:').lower()